...
rate_card <- tibble::data_frame(id = c("TA", "PM"),
                                price_in_original_currency = c(140, 70),
                                real_or_nominal = c("real", "real"),
                                currency = c("GBP", "GBP"),
                                price_frequency = c("week", "week"),
                                annual_percentage_increase = c(0.0,0.1)
)
...
test_that("get_staff_line_item", {
  ...
  sli <- get_staff_line_item("TA.1", staff_utilisation, rate_card, key_dates)
  expect_equal(sli[[1,"price_gbp"]], 20.0)
  expect_equal(sli[[1,"quantity"]], 0.5)
  ...
  staff_utilisation <- expand_staff_utilisation_to_time_horizon(staff_utilisation, key_dates)
  sli <- get_staff_line_item("PM", staff_utilisation, rate_card, key_dates)
})

rate_card <- tibble::data_frame(id = c("TA", "PM"),
                                price_in_original_currency = c(10, 50),
                                real_or_nominal = c("real", "real"),
                                currency = c("GBP", "GBP"),
                                price_frequency = c("working_day", "week"),
                                annual_percentage_increase = c(0.0,0)
                                )
